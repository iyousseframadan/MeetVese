...
export default function SignupPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setLoading(true);
    // [Logic] هنا سيتم ربط API التسجيل لاحقاً
    setTimeout(() => {
      setLoading(false);
      navigate("/home");
    }, 1500);
        ...
        >
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 dark:text-[#A8B0C2] text-sm mt-2">
              Join MeetVerse for a noise-free experience.
            </p>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    placeholder="John"
                    className="w-full bg-slate-50 dark:bg-[#0D0F16] border border-slate-200 dark:border-[#2A2E3B] rounded-2xl pl-12 pr-4 py-3.5 text-sm focus:border-blue-600 outline-none transition-all"
                  />
                </div>
              </div>
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  placeholder="Doe"
                  className="w-full bg-slate-50 dark:bg-[#0D0F16] border border-slate-200 dark:border-[#2A2E3B] rounded-2xl px-5 py-3.5 text-sm focus:border-blue-600 outline-none transition-all"
                />
              </div>
            </div>
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              />
              <span className="text-xs text-slate-500 dark:text-[#A8B0C2] group-hover:text-slate-800 dark:group-hover:text-white transition-colors">
                I agree to the{" "}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-800 disabled:opacity-70 text-white font-bold py-4 rounded-[1.5rem] shadow-xl shadow-blue-900/20 transition-all flex items-center justify-center gap-2 active:scale-95"
            >
              {loading ? (
                "Creating Account..."
              ) : (
